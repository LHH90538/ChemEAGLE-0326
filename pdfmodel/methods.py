...
LARGE_MODEL_ID = os.path.join(PACKAGE_DIR, "..", "safetensors", "visualheist-large")
BASE_MODEL_ID = os.path.join(PACKAGE_DIR, "..", "safetensors", "visualheist-base")
...
def _create_model(model_id, base_or_large):
    ...
    safetensors_download_path = os.path.join(package_dir, "..", "safetensors", safetensors_filename)
    if not os.path.exists(safetensors_download_path):
        raise FileNotFoundError(f"Local safetensors file not found: {safetensors_download_path}")

    state_dict = load_file(safetensors_download_path)
    model = AutoModelForCausalLM.from_pretrained(model_id, state_dict=state_dict, trust_remote_code=True, local_files_only=True)
