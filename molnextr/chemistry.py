...
def _llm_symbol_to_smiles(symbol: str, 
    ...
    Args:
        ...
        api_key: OpenAI API key (if None, uses API_KEY from module or environment variable)
        api_endpoint: OpenAI base URL (if None, uses BASE_URL from module or environment variable)
        api_version: (Deprecated/ignored) OpenAI client does not require Azure api_version
    ...
    api_key = api_key or globals().get("API_KEY") or os.getenv("apikey")
    api_endpoint = api_endpoint or globals().get("BASE_URL") or os.getenv("baseurl") or os.getenv("OPENAI_BASE_URL")

    # 如果没有 API key，静默返回 None（不启用 LLM）
    if not api_key:
        return None
...
def get_smiles_from_symbol(symbol, mol,atom, bonds, 
    ...
    if use_llm:
        ...
        has_api_config = (
            ...
            globals().get("BASE_URL") or
            os.getenv("apikey") or
            os.getenv("baseurl") or
